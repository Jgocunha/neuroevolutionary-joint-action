...
public:
  ...
  : rclcpp::Node("collaborative_assistance_scenario_v2")
  {
...
private:
  ...
  void run_script()
  {
    ...
    const auto sweep_duration = 2s;   // time for each 20->80 or 80->20 sweep
    const double start_pos = 80.0; // obj3
    const double end_pos   = 20.0; // obj1

    // (2) wait 5 s → all objects present, hand at 20
    sleep_or_stop(5s);
    if (!running_) return;
